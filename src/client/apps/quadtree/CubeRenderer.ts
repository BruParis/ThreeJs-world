...
export class CubeRenderer {
  ...
  // Generation ID to cancel outdated requests (only incremented on full reset)
  private currentGenerationId: number = 0;
  // Keys of quadrants currently being generated by workers (to avoid re-submitting)
  private inFlightWorkerKeys: Set<string> = new Set();
  ...
  async updateLODQuadrants(neededQuadrants: Map<string, QuadrantSpec>): Promise<void> {
    ...
    // Find quadrants to add: needed, not yet in scene, and not already in-flight to workers
    const toAdd: QuadrantSpec[] = [];
    for (const [key, spec] of neededQuadrants) {
      if (!this.hoverQuadrantMeshes.has(key) && !this.inFlightWorkerKeys.has(key)) {
        toAdd.push(spec);
      }
    }

    // If nothing new to submit, just remove obsolete meshes and return
    if (toAdd.length === 0) {
    ...
    if (this.useWorkers) {
      ...
      // Capture generation ID at submission time (only reset by clearQuadrantMeshes)
      const generationId = this.currentGenerationId;

      // Mark all submitted keys as in-flight so subsequent frames don't re-submit them
      for (const spec of toAdd) {
        this.inFlightWorkerKeys.add(spec.key);
      }
      ...
      try {
        await this.meshService.generateMeshesBatchedParallel(requests, (batchResults) => {
          for (const result of batchResults) {
            // Always remove from in-flight, even if this generation was reset
            this.inFlightWorkerKeys.delete(result.id);

            // Discard result if a full reset happened since submission
            if (generationId !== this.currentGenerationId) continue;
        ...
        // Remove obsolete meshes after new ones are in the scene
        if (generationId === this.currentGenerationId) {
          for (const key of toRemove) {
            const mesh = this.hoverQuadrantMeshes.get(key);
            if (mesh) {
        ...
        // isGeneratingMeshes is a legacy flag; keep it accurate
        if (generationId === this.currentGenerationId) {
          this.isGeneratingMeshes = false;
        }
      }
      ...
      console.log(`[CubeRenderer-Sync] Updated LOD quadrants - added ${newMeshes.length}, removed ${toRemove.length}, total: ${(performance.now() - start_time).toFixed(2)}ms`);
    }
  }
